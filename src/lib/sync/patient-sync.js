// src/lib/sync/patient-sync.js - FIXED VERSION with error handling
...
/**
 * Add a patient record to the sync queue - with error handling
 * @param {string|number} recordId Patient ID
...
export async function queuePatientSync(recordId, operation, data) {
	try {
		// Validate inputs
		if (!recordId) {
			console.warn('Cannot queue patient sync: invalid recordId', recordId);
			return null;
		}

		if (!operation || !['add', 'update', 'delete'].includes(operation)) {
			console.warn('Cannot queue patient sync: invalid operation', operation);
			return null;
		}

		return await syncQueueDb.addToQueue('patients', recordId, operation, data);
	} catch (error) {
		console.error('Error queueing patient for sync:', error);
